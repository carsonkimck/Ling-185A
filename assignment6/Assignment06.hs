...
insideTrees cfg str n = 
    let (nts, ts, i, r) = cfg in
        case str of 
            [] -> []
            (x:[]) -> let tProb = (r (TRule n x)) in case tProb of 
                0.0 -> []
                _ -> [(tProb, Leaf n x)]
            (x:y:rest) -> let conj i ld rd = let ntProb = (r (NTRule n (ld, rd))) in case ntProb of
                                0.0 -> []
                                _ -> liftA2(\(d1, t1) -> \(d2, t2) -> ((ntProb * (d1*d2)), NonLeaf n t1 t2)) (insideTrees cfg ((take i str)) ld)(insideTrees cfg (drop i str) rd) 
                                in concat (liftA3 conj [1 .. (length str - 1)] nts nts)

                                
completeTrees :: GenericCFG nt t Double -> [t] -> [(Double, Tree nt t)]
completeTrees cfg str = let (nts, ts, i, r) = cfg in 
          concat (map(\q -> convertDoubles(insideTrees cfg str q) (i q)) nts)

--- Helper function for completeTrees, multiplies all doubles by initial nonterminal probability ----- 
convertDoubles :: [(Double, Tree nt t)] -> Double -> [(Double, Tree nt t)] 
convertDoubles list double = case double of 
    0.0 -> []
    _ -> map(\(d1, t1) -> ((d1 * double), t1)) list
